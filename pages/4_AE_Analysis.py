...
AE_spindle_data = pd.read_excel(path, sheet_name=add_selectbox, usecols=[0, 6])
# print(AE_spindle_data)
c = alt.Chart(AE_spindle_data).mark_line().encode(x='index', y='AE_spindle',color=alt.value("#fc5185"))
st.altair_chart(c, use_container_width=True)

st.markdown('#### 台面噪声信号')
AE_table_data = pd.read_excel(path, sheet_name=add_selectbox, usecols=[0, 5])
# print(AE_spindle_data)
c = alt.Chart(AE_table_data).mark_line().encode(x='index', y='AE_table',color=alt.value("#086972"))
st.altair_chart(c, use_container_width=True)
